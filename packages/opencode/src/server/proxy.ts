...
function headers(req: Request, extra?: HeadersInit) {
  ...
  out.delete("x-shlifecode-directory")
  out.delete("x-shlifecode-workspace")
  if (!extra) return out
...
const app = (upgrade: UpgradeWebSocket) =>
  new Hono().get(
    "/__workspace_ws",
    upgrade((c) => {
      const url = c.req.header("x-shlifecode-proxy-url")
      const queue: Msg[] = []
...
export namespace ServerProxy {
  ...
  export function websocket(
    ...
    next.set("x-shlifecode-proxy-url", socket(target.url))
