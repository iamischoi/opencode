...
export function mergeWithClaudeCodeAgents(
  ...
  log("[subagent-discovery] loading agents", {
    serverAgentsCount: serverAgents.length,
    userAgentsCount: Object.keys(userAgentsRecord).length,
    projectAgentsCount: Object.keys(projectAgentsRecord).length,
  })
...
export function isTaskCallableAgentMode(mode: AgentMode): boolean {
  return mode === "all" || mode === "subagent" || mode === undefined
}
